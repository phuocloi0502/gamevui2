...
export const SPECIES_TEMPLATES: SpeciesTemplate[] = [
  ...
  },
  {
    ...
    slots: [
      ...
      upload('body', 'Toàn bộ cổ / thân chữ S / phần cuộn dưới', [layer('body', 5, -105, .5, .65, 2, .85)]),
      upload('head', 'Đầu (không chứa mắt)', [layer('head', 35, -190, .5, .8, 3, .8)]),
      ...eyeStates(.5, .8, 3.1),
      upload('jaw', 'Hàm', [layer('jaw', 58, -170, .45, .2, 4, .8, 'head')], true),
      upload('crest', 'Mào', [layer('crest', 30, -215, .5, .85, 5, .8, 'head')], true),
