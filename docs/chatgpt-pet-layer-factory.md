...
Một yêu cầu tạo pet nghĩa là tạo toàn bộ package artwork cần thiết:
...
Ưu tiên thứ tự generation để giữ identity: `head → head-closed → body → limbs → tail/appendages → species details → effects/projectile → particles → shadow`. Thứ tự này không phải z-order.
...
Không tự phát minh anatomy hoặc filename ngoài executable recipe. Với Fire Wolf, `mane.png` và `fang-effect.png` là optional nhưng thường đáng tạo vì củng cố identity Fire + Wolf.
...
| Species | Archetype / rig | Required PNG | Optional PNG | Identity và special rule |
|---|---|---|---|---|
| Fox | quadruped / `fox-quadruped` | `tail`, `rear-far`, `rear-near`, `body`, `front-far`, `front-near`, `head` | `shadow`, `effect-back`, `head-closed`, `effect-front`, `particles`, `projectile`; Level 3 tail slots | đầu chibi lớn, tai tam giác lớn, thân thấp, đuôi rất lớn; four-limb rule; projectile riêng |
| Wolf | quadruped / `quadruped-base` | `tail`, `rear-far`, `front-far`, `body`, `rear-near`, `front-near`, `head` | `shadow`, `mane`, `head-closed`, `jaw`, `fang-effect` | torso dài, mõm hẹp, vai/chân khỏe, tai nhỏ hơn Fox; four-limb rule |
| Bunny | hopper / `hopper-base` | `rear-ear`, `body`, `hind-leg`, `front-paw`, `head`, `front-ear`, `tail` | `shadow`, `head-closed`, `element-trail` | đầu tròn, tai rất dài, chân sau lớn; `hind-leg`/`front-paw` có thể tạo instance theo recipe |
| Turtle | tank / `tank-base` | `rear-feet`, `body`, `shell`, `front-feet`, `head` | `shadow`, `head-closed`, `shell-runes`, `element-aura` | thân thấp, mai rất rộng, đầu/chân ngắn; element tập trung ở shell/rune/cage |
| Golem | tank / `tank-base` | `rear-arm`, `rear-leg`, `torso`, `core`, `front-leg`, `front-arm`, `head` | `shadow`, `rock-fragments`, `ground-effect` | vai/tay rất lớn, đầu nhỏ, chân nặng, core rõ; material theo element |
| Dragon | winged / `winged-base` | `back-wing`, `tail`, `body`, `legs`, `front-wing`, `head` | `shadow`, `head-closed`, `horns`, `mouth-effect`, `element-aura` | đầu dragon lớn, hai cánh lớn, torso gọn, đuôi dài, sừng rõ |
| Owl | winged / `winged-base` | `back-wing`, `tail-feathers`, `body`, `talons`, `front-wing`, `head` | `shadow`, `head-closed`, `forehead-rune`, `orb-effect` | tròn/gọn, mắt và face disc lớn; không được đọc thành Hawk |
| Hawk | winged / `winged-base` | `rear-wing`, `tail-fan`, `body`, `talons`, `front-wing`, `head` | `shadow`, `head-closed`, `crest`, `wing-trail`, `vortex` | dài/aerodynamic, đầu nhọn, cánh dài; không được đọc thành Owl |
| Slime | blob / `blob-base` | `blob`, `face` | `shadow`, `inner-core`, `front-gloss`, `top-effect` | một blob thấp/tròn, không chân thường; silhouette blob ổn định |
| Serpent | serpent / `serpent-base` | `tail`, `body-lower`, `body-upper`, `head` | `shadow`, `head-closed`, `jaw`, `crest`, `mouth-effect` | thân S dài, lower body cuộn, không chân; head/neck/tail nối tự nhiên |
...
Z-order do `Layer.z` trong executable recipe quyết định, từ số nhỏ ở sau tới số lớn ở trước. Generation order không thay đổi z-order. `effect-front` là layer visual; `projectile.png` chỉ là attack asset riêng khi recipe khai báo projectile.
...
3. Chọn toàn bộ required slots và optional phù hợp như mane/fang effect.
