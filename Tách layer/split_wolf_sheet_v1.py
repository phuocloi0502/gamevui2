...
def slot_bounds(slot_id: int, width: int, height: int, margin: int = 0):
...
def empty_slot_bounds(width: int, height: int):
    return slot_bounds(EMPTY_SLOT_INDEX, width, height, margin=0)


def clear_empty_slot_alpha(alpha: np.ndarray):
    h, w = alpha.shape
    x0, y0, x1, y1 = empty_slot_bounds(w, h)

    cleaned = alpha.copy()
    contaminated = int(np.count_nonzero(cleaned[y0:y1, x0:x1] > 0))
    cleaned[y0:y1, x0:x1] = 0

    return cleaned, contaminated


def pixel_slot_ids(xs: np.ndarray, ys: np.ndarray, width: int, height: int):
    cols = np.minimum(xs * GRID_COLS // width, GRID_COLS - 1)
    rows = np.minimum(ys * GRID_ROWS // height, GRID_ROWS - 1)
    return rows * GRID_COLS + cols


def assign_core_components(alpha: np.ndarray, core_alpha: int, core_min_area: int):
    h, w = alpha.shape
    ...
    for component_id, sl in enumerate(objects, start=1):
        ...
        logical_slots = pixel_slot_ids(gx, gy, w, h)
        counts = np.bincount(logical_slots, minlength=16)

        counts[EMPTY_SLOT_INDEX] = 0
...
def fallback_missing_seeds(
    ...
    candidates_by_slot = {i: [] for i in range(len(SLOT_NAMES))}
    ...
    for component_id, sl in enumerate(objects, start=1):
        ...
        candidates_by_slot[slot_id].append((area, sl, local_component))
    ...
    for slot_id in range(len(SLOT_NAMES)):
        ...
        _, sl, local_component = max(candidates, key=lambda item: item[0])
...
def build_owner_map(
    ...
    if not valid_slots:
        raise RuntimeError("Không tìm thấy core nào trong Wolf sheet.")
...
def save_debug_grid(
    ...
    for row in range(1, GRID_ROWS):
        y = row * h // GRID_ROWS
        draw.line([(0, y), (w, y)], fill=(255, 0, 0, 255), width=2)
...
def validate_image(image: Image.Image, strict_size: bool):
    if strict_size and image.size != EXPECTED_SIZE:
        raise RuntimeError(
            f"Wolf production sheet phải là "
...
def export_layer(
    ...
    if len(xs) == 0:
        raise RuntimeError(f"Slot rỗng: {output_path.name}")
...
def split_sheet(
    ...
    *,
    ...
    validate_image(image, strict_size)
    ...
    if original_alpha.min() == 255:
        raise RuntimeError(
            ...
            "Wolf V2 không tự tạo alpha từ RGB."
        )

    alpha, contaminated_pixels = clear_empty_slot_alpha(original_alpha)

    if contaminated_pixels > 0:
        print(
            f"WARNING: slot 16 EMPTY có {contaminated_pixels} pixel alpha > 0. "
    ...
    if missing:
        raise RuntimeError(
            ...
            + "\n- ".join(missing)
            + "\n\nThử giảm --core-alpha hoặc --core-min-area."
        )
    ...
    for slot_id, filename in enumerate(SLOT_NAMES):
        ...
        print(f"{slot_id + 1:02d}. Saved: {filename}")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Split Wolf 4x4 production sheet into 15 PNG layers. "
            "Slot 16 is mandatory EMPTY. "
            "V2 uses overlap-based core assignment + ownership margin."
        )
    )
