#!/usr/bin/env python3
"""
split_dragon_sheet_v1.py

Tách Dragon Layer Sheet 3 cột × 4 hàng = 12 layer.
...
Chạy:
    python split_dragon_sheet_v1.py dragon.png --output out --zip out.zip
    ...
    *,
    ...
    if alpha.min() == 255:
        raise RuntimeError(
            ...
            "Dragon V1 không tự tạo alpha từ RGB."
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Split Dragon 3x4 sheet into 12 layers using high-alpha cores + nearest-core ownership."
    )

    parser.add_argument("input", type=Path, help="PNG Dragon sheet nguồn")
    parser.add_argument("--output", type=Path, default=Path("dragon-layer-sheet"), help="Thư mục output")
